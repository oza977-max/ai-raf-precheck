...
**Stub-path:** 
### O-003: TC-R3-JU-5-01 flaked under concurrent-agent CPU load, NOT because the test is slow — this entry's first diagnosis was wrong

**Tour:** feature
**Given:** the full suite run from inside an agent worktree while a second agent session was running its own suite on the same machine
**When:** `npm test` is run repeatedly
**Then:** `TC-R3-JU-5-01: the fields that block progress and the fields marked required are the same set` failed intermittently on a 5000ms timeout -- **3 of 6** full-suite runs at `34efcdb`, against **6 of 6** passing for the same file in isolation. The measurement was real. The diagnosis first recorded here was not: this entry originally concluded the test was inherently too slow, called it P8-C02's to fix, and proposed raising the timeout. That was inference presented as finding, in the exact shape BC-001 exists to stop -- a claim about existing code written from a symptom rather than checked against a cause. The real cause was found independently by the P8-C03 session and fixed at `90c5d3c`: agent worktrees under `.claude/worktrees/` hold a full copy of the repo, tests included, so vitest globbed both copies, every count doubled, and the two copies competed for CPU until interaction-heavy tests timed out. After merging that fix the suite ran **6 of 6** clean here, with no change to the test. Nothing about TC-R3-JU-5-01 needed fixing. Standing lesson, alongside charter 004's O-003: a flake observed while another agent is working the same machine is a claim about the environment until proven otherwise, and 'which change is in the tree' is the least likely explanation when two sessions share a CPU.
**Stub-path:** 
