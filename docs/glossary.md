...
| **Rule challenge** | A 2LoD reviewer's formal, permanent objection to a *rule* — not to a use case. Filing one changes nothing about any verdict; it lands in the rule-improvement queue for the people who author the rulebook. The design keeps one principle absolute: a dissent never overrides a decision. |
| **Rule-improvement queue** | The **Rule challenges** screen — every filed challenge, grouped by rule, derived from the audit trail. Evidence for the humans who maintain the rules; nothing in it feeds back into the engine. |
