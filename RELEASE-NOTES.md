...
First tagged release of AIGate. Developer-facing notes; the user-facing
version is `RELEASE-NOTES.html`, and the full history is `CHANGELOG.md`.
...
The whole product is new. Grouped by requirement domain; 56 requirements are
in V1 scope and 54 are implemented end to end.
...
- `PE-2`/`PE-3` Track and tier assignment, short-circuiting at the first
  matching rule.
...
- **`PE-6`/`RA-2` track floors are not implemented.** Tier floors work.
  Track floors were deliberately not built — the supplement model was chosen
  instead (`src/engine/types.ts:205`). The requirement wording was overtaken
  by the design decision and should be amended to match.
- **`specs/evaluation-engine.md` and its `.html` twin disagree** about
  whether the `track_floor` effect type exists. `spec-parity-check.py` reads
  only `.md` and cannot see this.
- **Four MUST property tests were specified and never written**
  (`TC-PE-PROPERTY-01`–`04`). They name `fast-check`, which is not a
  dependency. The behaviour is covered by example tests; the universal
  quantifier is not.
- **Acceptance-criterion traceability is 82/158.** The remaining 76 are
  covered but do not carry a trace ID.
...
**Test status.** 376 automated tests across 39 files, three consecutive green
runs. `npx tsc --noEmit` clean, `npm run build` clean,
`python3 scripts/spec-parity-check.py` clean (R1–R8). All 158 acceptance
cases walked with per-case evidence: 152 PASS, 2 FAIL, 4 STUB, 0 BLOCKED.

**Release verdict inputs.** No VV-4 trigger fired. Every VV-3 criterion
passed. `VV-2(a)` failed on traceability and the four unwritten property
tests, which is what caps the verdict at Demo-ready.
